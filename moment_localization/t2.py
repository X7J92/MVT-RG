...
import sys
sys.path.insert(0, '/home/l/data_2/wmz/STA/DepNet_ANet_Release')
...
def parse_args():
    ...
    # general
    parser.add_argument('--cfg', help='experiment configure file name', default='/home/l/data_2/wmz/STA/DepNet_ANet_Release/experiments/dense_charades/charades.yaml',required=False, type=str)
...
if __name__ == '__main__':
    ...
    def network(sample):
        ...
        sentence_mask = sample['batch_sentence_mask'].cuda()
        ...
        with torch.no_grad():  # 禁用梯度更新
            for idx in tqdm(range(len(anno_idxs)), desc="Computing query2video scores"):
                ...
                for i in range(0, len(anno_idxs), bsz):
                    ...
                    fused_h, map_mask, txt_h, txt_h_a, _ = model.fusion_layer(
                        textual_input_batch, textual_mask_batch, sentence_mask_batch,
                        map_h, map_mask
                    )
                    fused_h = fused_h.view(batch_size * 8, 512, map_size, map_size)

                    map_mask = map_mask.view(batch_size * 8, 1, map_size, map_size)
                    sentence_mask_batch = sentence_mask_batch.view(batch_size * 8, 1)
                    sentence_mask_batch = sentence_mask_batch[:, :, None, None]
                    ...
                    prediction = model.pred_layer1(fused_h)
                    prediction = prediction * map_mask
                    ...
                    mask3 = sentence_mask_batch.view(batch_size, 8).bool().unsqueeze(-1).unsqueeze(-1).expand_as(
                        indices)
                    ...
                    fused_h_reshaped = fused_h.view(batch_size, 8, 512, 16, 16)
                    ...
                    vg_hs_v = model.vl_transformer1(vl_src, vl_mask, vl_pos_v)
                    vg_hs_v = vg_hs_v[0]
                    ...
                    vg_hs_t = model.vl_transformer1(vl_src_t, vl_mask_t, vl_pos_t)
                    vg_hs_t = vg_hs_t[0]
                    visual_output_g = visual_output_g.permute(2, 0, 1)
                    video_mask = torch.full((batch_size, 16), False).cuda()
                    ...
                    vg_hs_video = model.vl_transformer1(vl_src_video, vl_mask_video, vl_pos_video)
                    ...
                    sim2_tensor = torch.diag(sims)
                    sims_list_batch.extend(sim2_tensor.tolist())
                    joint_prob_batch.append(joint_prob.cpu())  # 保存joint_prob到CPU

                # 保存中间结果到硬盘
                temp_sims_file = os.path.join(temp_dir, f'sims_list_{idx}.pt')
                temp_joint_prob_file = os.path.join(temp_dir, f'joint_prob_{idx}.pt')
                torch.save(sims_list_batch, temp_sims_file)
                torch.save(joint_prob_batch, temp_joint_prob_file)

        # 从硬盘加载所有中间结果
        sims_list = []
        joint_prob_list = []
        for idx in range(len(anno_idxs)):
            temp_sims_file = os.path.join(temp_dir, f'sims_list_{idx}.pt')
            temp_joint_prob_file = os.path.join(temp_dir, f'joint_prob_{idx}.pt')
            sims_list.extend(torch.load(temp_sims_file))
            joint_prob_list.extend(torch.load(temp_joint_prob_file))

        joint_prob_tensor = torch.stack(joint_prob_list, dim=0)
        sims_list_tensor = torch.tensor(sims_list).detach()
        sorted_times = get_proposal_results(joint_prob_tensor, duration)
        loss_value = 1

        # 清理临时文件
        # for idx in range(len(anno_idxs)):
        #     os.remove(os.path.join(temp_dir, f'sims_list_{idx}.pt'))
        #     os.remove(os.path.join(temp_dir, f'joint_prob_{idx}.pt'))
    ...
    def on_test_end(state):
        ...
        sims_l= sims_l.reshape(1227,1227)
        print(sims_l.shape)
