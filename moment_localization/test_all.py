...
if __name__ == '__main__':
    ...
    def network(sample):
        ...
        sims_list=[]
        joint_prob_list=[]
        # print(len(anno_idxs))4865
        for idx in tqdm(range(len(anno_idxs)), desc="Computing query2video scores"):

          # weights_list=weights_list[idx].unsqueeze(0).repeat(bsz, 1, 1,1)
          # ids_list=ids_list[idx].unsqueeze(0).repeat(bsz, 1, 1)
          sims_list_batch=[]
          joint_prob_batch=[]
          # for i in tqdm(range(0, len(anno_idxs) - bsz + 1, bsz), desc="Processing one by one"):
          for i in range(0, len(anno_idxs), bsz):

              visual_input_batch = visual_input[i:i + bsz]
              batch_size = min(bsz, len(anno_idxs) - i)  # 计算实际的批大小，尤其是最后一个批次可能不足bsz
              ...
              map_h, map_mask = model.prop_layer(vis_h)  # map_h (b,512,64,64) map_mask (b,1,64,64)
              map_h = model.bmn_layer(vis_h)
              map_size = map_h.size(3)
              fused_h, map_mask, txt_h, txt_h_a, _ = model.fusion_layer(textual_input_batch, textual_mask_batch, sentence_mask_batch,
                                                                       map_h, map_mask)
              ...
              sentence_mask_batch = sentence_mask_batch[:, :, None, None]  # sentence_mask (b*8,1, 1, 1)
              ...
              fused_h = model.map_layer(fused_h, map_mask)  # fused_h (b,512,8,64,64)
              ...
              prediction = model.pred_layer1(fused_h)  # prediction (b*K,1,64,64)
              ...
              vg_hs_v = model.vl_transformer1(vl_src, vl_mask, vl_pos_v)  # (1+L+N)xBxC
              vg_hs_v = vg_hs_v[0]
              # print(vg_hs_v.shape)
              #######################################################################################################################################################################
              # 计算全局文本信息
              ...
              vg_hs_t = model.vl_transformer1(vl_src_t, vl_mask_t, vl_pos_t)  # (1+L+N)xBxC
              vg_hs_t = vg_hs_t[0]
              # print(vg_hs_t.shape)
              ##############################################################################################################
              ...
              vg_hs_video = model.vl_transformer1(vl_src_video, vl_mask_video, vl_pos_video)  # (1+L+N)xBxC
              ...
              # visual_output_g=visual_output_g/visual_output_g.norm(dim=-1, keepdim=True)
              # sims = torch.matmul(vg_hs_v, vg_hs_t.T)
              sims2 = torch.matmul(vg_hs_video, vg_hs_t.T)
              # joint_prob1 = joint_prob.detach()
              sim2_tensor = torch.diag(sims2).detach().cpu()
              sims_list_batch.extend(sim2_tensor.tolist())
              joint_prob1 = joint_prob.detach()
              joint_prob_batch.append(joint_prob1)
          sims_list.append(sims_list_batch)
          joint_prob_tensor0 = torch.cat(joint_prob_batch, dim=0).detach().cpu()
          # print(joint_prob_tensor0.shape)torch.Size([g, 8, 1, 32, 32])
          joint_prob_tensor_idx = joint_prob_tensor0[idx].unsqueeze(0).detach().cpu()
          joint_prob_list.append(joint_prob_tensor_idx)

        joint_prob_tensor = torch.cat(joint_prob_list, dim=0).detach().cpu()
        print(joint_prob_tensor.shape)
        sims_list_tensor = torch.tensor(sims_list).detach().cpu()  # (g,g)
        # print(joint_prob_tensor.shape)torch.Size([g, 8, 1, 32, 32])
        # new_duration = duration[:64]
